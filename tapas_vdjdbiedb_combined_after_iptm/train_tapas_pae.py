...
if 'sample_id' not in df_metrics.columns:
    print("ERROR: PAE CSV is missing the sample_id column.", file=sys.stderr)
    sys.exit(1)

df_metrics = df_metrics.rename(columns={'sample_id': 'pdb_id'})
missing = [c for c in FEATURE_COLS if c not in df_metrics.columns]
if missing:
    print(f"ERROR: PAE CSV is missing required columns: {missing}", file=sys.stderr)
    sys.exit(1)
...
def run_cv_evaluation(folder_name, df_metrics):
    ...
    for i in range(5):
        ...
        if len(train_df) == 0 or len(test_df) == 0:
            print(
                f"ERROR: {folder_name} fold{i} became empty (train or test) after merging PAE.",
                file=sys.stderr,
            )
            sys.exit(1)
        if len(train_df) != len(df_train_f) or len(test_df) != len(df_test_f):
            print(
                f"ERROR: {folder_name} fold{i}: row count mismatch after merging PAE "
                f"(train {len(df_train_f)}→{len(train_df)}, test {len(df_test_f)}→{len(test_df)}). "
                'fold ids may be missing from the PAE CSV, or pdb_id may be duplicated.',
                file=sys.stderr,
            )
            sys.exit(1)
