...
def calculate_pdockq_direct(model_file, receptor_chains, ligand_chains, distance_threshold=8.0):
    # Parser: support both CIF and PDB
    if model_file.endswith(".pdb"):
        parser = PDB.PDBParser(QUIET=True)
    else:
        parser = PDB.MMCIFParser(QUIET=True)
    ...
    def get_coords_and_plddts(chain_ids):
        ...
        for chain_id in chain_ids:
            ...
            for res in model[chain_id]:
                # Standard amino acids only (exclude water / hetero atoms)
                if res.id[0] != ' ':
                    continue
                # Glycine: CA; others: CB (FoldDock pDockQ convention)
                if 'CB' in res:
                    atom = res['CB']
                elif 'CA' in res:
                    atom = res['CA']
    ...
    # 1. Extract atom coordinates and pLDDT for both groups
    ...
    # 3. Interface contacts (within distance_threshold Angstrom)
    ...
    # 4. Mean pLDDT at interface residues
    ...
    # 5. pDockQ formula (Bryant et al., 2022)
...
# Notes: (1) use the original un-merged model file; (2) restrict to the TCR–pMHC interface.
...
def main(model_folder, output_folder): 
    ...
    csv_path = f"{output_folder}/results_model_quality_metrics.csv"

    # Resume: skip (pdb_id, model_number) pairs already present in the output CSV
    already_done = []
    ...
    ids_0 = [x.split('/')[-1] for x in glob.glob('/home/ha01994/af3_structpred/af3_vdjdb0/af_output/*')]
    ids_1 = [x.split('/')[-1] for x in glob.glob('/home/ha01994/af3_structpred/af3_vdjdb1/af_output/*')]
    ids_2 = [x.split('/')[-1] for x in glob.glob('/home/ha01994/af3_structpred/af3_vdjdb2/af_output/*')]
    ids_3 = [x.split('/')[-1] for x in glob.glob('/home/ha01994/af3_structpred/af3_vdjdb3/af_output/*')]
    pdb_ids = ids_0 + ids_1 + ids_2 + ids_3
    ...
    for pdb_id in pdb_ids:
        ...
        for model_number in range(5):
            ...
            if pdb_id in ids_0: foldername = 'vdjdb0'                    
            if pdb_id in ids_1: foldername = 'vdjdb1'                    
            if pdb_id in ids_2: foldername = 'vdjdb2'                    
            if pdb_id in ids_3: foldername = 'vdjdb3'

            pdb_model = f'/home/ha01994/af3_structpred/af3_{foldername}/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_model.cif'
            summary_json = f'/home/ha01994/af3_structpred/af3_{foldername}/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_summary_confidences.json'
            all_data_json = f'/home/ha01994/af3_structpred/af3_{foldername}/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_confidences.json'
            ...
            # Pass the original model CIF directly to the in-process pDockQ routine
