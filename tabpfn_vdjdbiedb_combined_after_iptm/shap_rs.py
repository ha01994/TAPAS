...
DEVICE      = 'cuda:0'
FOLDER_NAME = 'dataset_iptm_filtered_rs'
FOLD_IDX    = 0
...
if not pae_dfs:
    raise FileNotFoundError(
        f"No PAE CSV found. Expected at least one of: {', '.join(pae_files)}"
    )
...
import pickle
with open(f'shap_values_rs_fold{FOLD_IDX}.pkl', 'wb') as f:
...
plt.tight_layout()
plt.savefig(f'shap_beeswarm_rs_fold{FOLD_IDX}.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(f'shap_bar_rs_fold{FOLD_IDX}.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(f'shap_waterfall_rs_fold{FOLD_IDX}.png', dpi=300, bbox_inches='tight')
plt.close()
...
summary_df.to_csv(f'shap_summary_rs_fold{FOLD_IDX}.csv', index=False)
