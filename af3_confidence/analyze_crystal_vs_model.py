...
def main(crystal_folder, model_folder, output_folder): 
    ...
    pdb_tcr_root = os.environ.get("TAPAS_PDB_TCR_ROOT", ".")
    crystal_root = os.path.join(pdb_tcr_root, crystal_folder)
    model_root = os.path.join(pdb_tcr_root, model_folder)
    output_root = os.path.join(pdb_tcr_root, output_folder)
    pdbs = [x.split('/')[-1] for x in glob.glob(f'{model_root}/models/*')]
    pdbs.sort()
    ...
    for pdb_id in pdbs:
        print(f'==================={pdb_id}=====================')
        crystal_pdb = f'{crystal_root}/crystals1/{pdb_id}.pdb' #cleaned
        dockq_native = f'{crystal_root}/crystals3/{pdb_id}.pdb' #merged

        for num in range(5):
            num = str(num)
            print('model', num)
            model_pdb = f'{model_root}/models1/{pdb_id}_model_{num}.pdb' #cleaned        
            dockq_model = f'{model_root}/models3_AB/{pdb_id}_model_{num}.pdb' #merged
    ...
    df.to_csv(os.path.join(output_root, "results_crystal_vs_model.csv"), index=False)
