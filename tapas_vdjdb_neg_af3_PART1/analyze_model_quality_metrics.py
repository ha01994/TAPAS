...
# Allow restarting after interruption (finished jobs recorded in the output CSV).
def main(model_folder, output_folder): 
    os.makedirs(f'{output_folder}', exist_ok=True)    
    
    csv_path = f"{output_folder}/results_model_quality_metrics_part1.csv"
            
    pdb_ids = [x.split('/')[-1] for x in glob.glob('/home/ha01994/af3_structpred/af_neg_json_part1/af_output/*')]
    ...
    for pdb_id in pdb_ids:
        ...
        for model_number in range(5):
            ...
            # Skip if this pdb_id / model_number was already written
            if f"{pdb_id}_model_{model_number}" in already_done:
                print(f"Skipping: {pdb_id} model {model_number} (already processed)")
                continue                

            pdb_model = f'/home/ha01994/af3_structpred/af_neg_json_part1/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_model.cif'
            summary_json = f'/home/ha01994/af3_structpred/af_neg_json_part1/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_summary_confidences.json'
            all_data_json = f'/home/ha01994/af3_structpred/af_neg_json_part1/af_output/{pdb_id}/seed-1_sample-{model_number}/{pdb_id}_seed-1_sample-{model_number}_confidences.json'
            ...
            b_factors_cdr2_B, b_factors_cdr3_B = cdr_plddts(pdb_model, TRA_CHAIN, TRB_CHAIN)
